...
WHERE PP1.PRPT_ID=#arguments.PROPERTY_ID#
    ...
    <cffunction name="getPropertyDetailsWithCatId"  access="remote" httpMethod="Post" returntype="any" returnFormat="json">
        ...
        <cfquery name="getAll" datasource="#dsn#">
            SELECT DISTINCT PPD.PROPERTY_DETAIL,PPD.PROPERTY_DETAIL_ID,T.PRPT,PP.PROPERTY,PP.PROPERTY_ID FROM CatalystQA_product.PRODUCT_DT_PROPERTIES  PDP
...
WHERE PRODUCT_ID IN (SELECT PRODUCT_ID FROM CatalystQA_product.PRODUCT WHERE PRODUCT_CATID=#arguments.PRODUCT_CATID#) <cfif len(arguments.RELATED_PROP_ID)>AND PPD.PRPT_ID IN (0#arguments.RELATED_PROP_ID)#<cfelse> AND PPD.PRPT_ID=#arguments.PROPERTY_ID#</cfif>
<cfif len(arguments.RELATED_VAR_ID)> AND PPD.RELATED_VARIATION_ID LIKE '%#arguments.RELATED_VAR_ID#%' </cfif>
        </cfquery>
          <cfset ReturnArr=arrayNew(1)>
          <cfloop query="getAll">
              <cfscript>
