...
WHERE PP1.PRPT_ID=#arguments.PROPERTY_ID# 
    ...
    <cffunction name="getWesselProducts" access="remote" httpMethod="Post" returntype="any" returnFormat="json">
        ...
        <cfquery name="getProd" datasource="#dsn#">
            select S.PRODUCT_ID,S.STOCK_ID,S.MANUFACT_CODE,S.PRODUCT_NAME,POR.PROP_LIST,POR.JSON_STRINGIM,PU.MAIN_UNIT,S.PRODUCT_CODE,S.TAX,S.PRODUCT_CODE_2,PC.PRODUCT_CAT,PC.PRODUCT_CATID
            (
 ...
 LEFT JOIN CatalystQA_product.PRODUCT_CAT_PROPERTY AS PCP ON PCP.PRODUCT_CAT_ID=S.PRODUCT_CAT_ID AND PCP.PROPERTY_ID=PP.PROPERTY_ID
         WHERE PDP.PRODUCT_ID=S.PRODUCT_ID AND PDP.VARIATION_ID IS NOT NULL  for JSON AUTO
            ) AS PC_JSOIN
        ...
        <CFSET P.PC_JSOIN=deserializeJSON(getprod.PC_JSOIN)>
        <cfscript>
