...
where FOR_OFFER_ID=#arguments.OFFER_ID#
    ...
    <cfloop query="OFFER_HEADER">
        <cfquery name="OFFER_ROWS" datasource="#DSN3#">
            SELECT DISTINCT  S.PRODUCT_NAME,S.PRODUCT_ID,S.STOCK_ID,S.MANUFACT_CODE,S.PRODUCT_CODE,UNIQUE_RELATION_ID,PU.MAIN_UNIT,CONVERT(decimal(18,2),POR.QUANTITY) QUANTITY,CONVERT(DECIMAL(18,4),POR.PRICE) PRICE,CONVERT(DECIMAL(18,4),POR.PRICE_OTHER) PRICE_OTHER ,POR.OTHER_MONEY 
...
<cfquery name="getOffer" datasource="#dsn3#">
    SELECT C.NICKNAME,C.OZEL_KOD,C.FULLNAME,C.COMPANY_EMAIL,DELIVERDATE,OFFER_ID,DELIVER_FEE,TAX_STATUS,GENERAL_DISCOUNT_RATE,OTHER_MONEY,OFFER_STAGE,OFFER_DATE,PO.REF_NO,E.EMPLOYEE_NAME,E.EMPLOYEE_SURNAME,PO.OFFER_NUMBER,PO.DELIVERY_ADDRESS
		,DP.DELIVERY_PLACE,PO.OTHER_MONEY_VALUE ,PO.OTHER_MONEY  FROM CatalystQA_1.PBS_OFFER AS PO 
LEFT JOIN CatalystQA.COMPANY AS C ON C.COMPANY_ID IN(REPLACE(PO.OFFER_TO,',',''))
...
<cffunction name="SAVE_PURCHASE_OFFER_PRICES" access="remote" httpMethod="Post" returntype="any" returnFormat="json">
    ...
    <cfquery name="UP" datasource="#DSN3#">
        UPDATE PBS_OFFER SET NETTOTAL=#FormData.OFFER_FOOTER.brut_total_wanted_#,PRICE=#FormData.OFFER_FOOTER.brut_total_wanted_#,OTHER_MONEY_VALUE=#FormData.OFFER_FOOTER.net_total_wanted#,OTHER_MONEY='#FORMDATA.AKTIF_KUR#',DELIVERDATE=<CFIF LEN(FORMDATA.DELIVERY_TIMES)>'#dateFormat(dateAdd("d",FORMDATA.DELIVERY_TIMES,NOW()),"yyyy-mm-dd")#'<CFELSE>NULL</CFIF>,
