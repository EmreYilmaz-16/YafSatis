...
where FOR_OFFER_ID=#arguments.OFFER_ID#
    ...
    <cfloop query="OFFER_HEADER">
        <cfquery name="OFFER_ROWS" datasource="#DSN3#">
            SELECT S.PRODUCT_NAME,S.PRODUCT_ID,S.STOCK_ID,S.MANUFACT_CODE,S.PRODUCT_CODE,UNIQUE_RELATION_ID,PU.MAIN_UNIT,CONVERT(decimal(18,2),POR.QUANTITY) QUANTITY,CONVERT(DECIMAL(18,4),POR.PRICE) PRICE 
            FROM CatalystQA_1.PBS_OFFER_ROW AS POR 
            LEFT JOIN CatalystQA_1.STOCKS AS S ON S.STOCK_ID=POR.STOCK_ID
            LEFT JOIN CatalystQA_1.STOCKS as PU ON PU.PRODUCT_ID=S.PRODUCT_ID AND PU.IS_MAIN=1
