...
<cfquery name="getOffer" datasource="#dsn3#">
    SELECT C.NICKNAME,C.OZEL_KOD,C.FULLNAME,C.COMPANY_EMAIL,DELIVERDATE,OFFER_ID,DELIVER_FEE,TAX_STATUS,GENERAL_DISCOUNT_RATE,OTHER_MONEY,OFFER_STAGE,OFFER_DATE,PO.REF_NO,E.EMPLOYEE_NAME,E.EMPLOYEE_SURNAME,PO.DELIVERY_ADDRESS
		,DP.DELIVERY_PLACE  FROM CatalystQA_1.PBS_OFFER AS PO 
LEFT JOIN CatalystQA.COMPANY AS C ON C.COMPANY_ID IN(REPLACE(PO.OFFER_TO,',',''))
...
<CFSET OFFER_DATA.COMPANY_EMAIL=COMPANY_EMAIL>
