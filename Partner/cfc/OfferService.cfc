...
<cfquery name="getOffer" datasource="#dsn3#">
    SELECT C.NICKNAME,C.FULLNAME,DELIVERDATE,OFFER_ID,DELIVER_FEE,TAX_STATUS,GENERAL_DISCOUNT_RATE,OTHER_MONEY,OFFER_STAGE,OFFER_DATE,PO.REF_NO  FROM CatalystQA_1.PBS_OFFER AS PO 
LEFT JOIN CatalystQA.COMPANY AS C ON C.COMPANY_ID IN(REPLACE(PO.OFFER_TO,',',''))
